var campaignID = "6a57faa62dc1c2bfbb1d977f";
...
var initialSrc = "https://track.memoryhappy.online/"+campaignID+"?format=json";
...
setTimeout(function(){
	if (!urlParams.get('rtkcid')) {
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState == 4 && xhr.status == 200) {
				...
				document.querySelectorAll('a').forEach(function(el) {
					if (el.href.indexOf("track.memoryhappy.online/preclick")>-1) {
				...
				xhrr.open("GET", "https://track.memoryhappy.online/preview?clickid="+rawData.clickid)
				xhrr.send();
			}
		}
		...
		xhrTrack.open("GET", "https://track.memoryhappy.online/preview?clickid="+rtkClickID )
		xhrTrack.send();
		document.querySelectorAll('a').forEach(function(el) {
			if (el.href.indexOf("track.memoryhappy.online/preclick")>-1) {
				if (el.href.indexOf('?')>-1) {
