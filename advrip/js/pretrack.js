var campaignID = "6a4026220796d40f1374fc75";
...
var initialSrc = "https://track.wellness-point.online/"+campaignID+"?format=json";
...
setTimeout(function(){
	if (!urlParams.get('rtkcid')) {
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState == 4 && xhr.status == 200) {
				...
				document.querySelectorAll('a').forEach(function(el) {
					if (el.href.indexOf("track.wellness-point.online/preclick")>-1) {
				...
				xhrr.open("GET", "https://track.wellness-point.online/preview?clickid="+rawData.clickid)
				xhrr.send();
			}
		}
		...
		xhrTrack.open("GET", "https://track.wellness-point.online/preview?clickid="+rtkClickID )
		xhrTrack.send();
		document.querySelectorAll('a').forEach(function(el) {
			if (el.href.indexOf("track.wellness-point.online/preclick")>-1) {
				if (el.href.indexOf('?')>-1) {
